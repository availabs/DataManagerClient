...
const SedMapFilter2 = ({ source, activeVar, setActiveVar }) => {
  let varType = useMemo(
    () =>
...
const SedTableFilter2 = ({ source, filters, setFilters }) => {
...
const SedTableTransform2 = (tableData, attributes, filters, flag) => {
  ...
  const getSum = (accessor, summarizeKeys, groupByTableData) => {
    ...
    (summarizeKeys || []).forEach((k) => {
      ...
      sum += Math.floor(
        sumBy(selectedCounty, `${accessor}`) || 0
      )
    })
    return sum;
  }
  ...
  if (flag === "group_by_county") {
    if (summarize !== "county") {
      const keys = summarize === "subRegion" ? regionalData?.sub_regions : regionalData?.regions;
...
export { SedMapFilter2, SedTableFilter2, SedTableTransform2 };
